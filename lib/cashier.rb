# Summarizes and calculates the total price of the scanned products
# using a pricing rule if present for each set of products
class Cashier
  def initialize(scanned_products, rules)
    # Group and summarize the scanned products
    @products_summary = scanned_products.group_by { |i| i }
                                        .map { |k, v| { product_code: k, qty: v.count } }
    @rules = rules
    @total_price = 0
  end
  ...
  def calculate_total
    ...
    @products_summary.each do |product|
      ...
      regular_price = calc_regular_price(product[:product_code], product[:qty])

      # See which discount/pricing rule can be applied
  ...
  def calc_regular_price(product_code, product_qty)
